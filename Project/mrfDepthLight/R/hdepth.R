hdepth <- function(x, z = NULL, options = list() ){
  ...
  #Do the calculations
    ...
    Result <- .Fortran("HSDEP2",
                     ...
                     as.integer(n1),               #6 Number of observations in x.
    ...
    ReturnedResult <- list(depthX = NULL,
                         depthZ = Result[[7]],
                         singularSubsets = NULL,
                         dimension = NULL,
                         hyperplane = NULL,
    ...
    Result <- .Fortran("HSDEP3",
                     as.double(z[,1,drop = TRUE]), #1 First coordinate of points to compute the depth of.
                     as.double(z[,2,drop = TRUE]), #2 Second coordinate of points to compute the depth of.
                     as.double(z[,3,drop = TRUE]), #3 Third coordinate of points to compute the depth of.
                     as.integer(n2),               #4 Number of points in the set z.
                     as.double(x[,1,drop = TRUE]), #5 First coordinate of the data matrix x.
                     as.double(x[,2,drop = TRUE]), #6 Second coordinate of the data matrix x.
                     as.double(x[,3,drop = TRUE]), #7 Third coordinate of the data matrix x.
                     as.integer(n1),               #8 Number of observations in x.
                     as.double(rep(0,n2)),         #9 Halfspace depth of the points in z.
                     as.double(rep(0,n2)),         #10 Flag signaling exact fit.
                     PACKAGE = "mrfDepthLight")
    ReturnedResult <- list(depthX = NULL,
                         depthZ = Result[[9]],
                         singularSubsets = NULL,
    ...
    Result <- .C("HSDND",
               as.integer(n1 + n2),        #1 Total number of points for x and z combined.
               ...
               as.integer(n1),             #8 Number of points in the data matrix x.
               as.integer(CalcExact),      #9 Flag indicating whether exact computation should be performed.
               as.integer(seed),           #10 The random seed.
               PACKAGE = "mrfDepthLight")
    ReturnedResult <- list(depthX = (Result[[5]][1:n1]/n1),
