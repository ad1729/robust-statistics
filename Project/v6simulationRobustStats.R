...
for (i in 1:n_sim){
    ...
    outliers1 <- sample(1:n_obs, epsilon*n_obs)
    outliers2_1 <- sample(1:n_obs, epsilon*n_obs/2)
    outliers2_2 <- sample(1:n_obs, epsilon*n_obs/2)
    
    g1[outliers1,] <- mvrnorm(n= length(outliers1), mu = rep(10, 2), Sigma = matrix(c(1, 0, 0, 1), 2, 2))
    g2_1[outliers2_1] = rexp(n = length(outliers2_1), rate = 0.1)
    g2_2[outliers2_2] = rexp(n = length(outliers2_2), rate = 0.1)
...
#        ylab = '% Missclassification', yaxt='n')
