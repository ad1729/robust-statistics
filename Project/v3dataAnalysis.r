...
employed = read.csv('Hubert2010_emp.csv', header = F)
unemployed = read.csv('Hubert2010_unemp.csv', header = F)
...
for (i in 1:n_sim) {  
  ...
  for (x in k) {
    prediction <- knn(train[,-12], test[,-12], train[,12], k = x)
    accuracy[x] <- mean(prediction == test[,12])
  }
  ...
  knn_class = knn(train[,-12], test[,-12], train[,12], k = k_best)
  # summary(sdo_class)
  
  knn_results = table(test[,12], knn_class)
...
label_vec = c("SDO", "BD", "AO", "DO", "kNN", "QDA", "SVM")
